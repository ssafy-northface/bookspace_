...
### 김도희
처음에는 기술적으로 구현이 어렵다고 느껴졌던 부분들도 페어와 서로 생각을 공유하며 문제를 다시 정의하자 접근할 수 있는 방향이 보이기 시작했다. 생각보다 구현 난도가 높은 지점에서는 막연함이 컸지만,어떤 부분이 어려운지, 무엇이 핵심인지 짚어 나가면서 문제에 집중했다. 해결이 쉽지 않은 상황에서는 주변 사람들의 조언을 듣고, 그중 어떤 방법이 우리 프로젝트에 가장 적합한지 페어와 함께 깊이 생각해보는 과정 속에서 많은 것을 배울 수 있었다.
